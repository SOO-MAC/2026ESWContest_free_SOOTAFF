...
작업 후 테이블을 다시 인식하여 부족한 물품만 추가적으로 배치합니다.

### MODE 2 — Q&A Assistant 🎤

GUI에서 사회자가 질문자의 좌석을 선택하면 해당 위치로 이동합니다.

손의 위치와 거리를 인식하여 질문자에게 추가적으로 접근한 뒤
마이크를 전달하고, 질문 종료 후 다시 마이크를 회수합니다.

### MODE 3 — Table Cleanup 🧹

행사 종료 후 테이블을 순회하며 남아 있는 쓰레기 및 놓고 간 귀중품을 정리합니다.

`Name Tag → Valuables → Trash`

ArUco Marker를 이용해 명찰을 둘 위치를 찾고,
