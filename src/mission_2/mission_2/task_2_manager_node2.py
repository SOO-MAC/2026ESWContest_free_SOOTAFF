...
def wait_waypoint_tracking(
    ...
    while time.monotonic() < deadline:
        ...
        current_error_ratio = float(
            np.max(np.abs(last_errors) / safe_thresholds)
        )
...
class MotorControlNode(Node):
            ...
            try:
                with self.motion_lock:
                    reached = recover_to_start_after_collision(
                        self.port, self.packet, self.arm_writer
                    )
                self.get_logger().warn(
                    "[EMERGENCY RECOVERY] START 위치 복귀 완료 | "
                    f"ticks={reached.tolist()}"
                )
                response.message = f"{error} | START recovery completed"
                response.final_joint_ticks = [int(v) for v in reached]
            except Exception as recovery_error:
                response.message = f"{error} | recovery failed: {recovery_error}"

            try:
                response.final_gripper_tick = int(
                    read_gripper_position(self.port, self.packet)
                )
            except Exception:
    ...
    def handle_deliver_mike(
        ...
        """Task 2 마이크 전달용 J1~J5 tick 경로만 실행한다.

        그리퍼 동작은 포함하지 않는다. Arm Control이 basket pick, close,
        hand 이동, 3초 대기, release 순서를 관리한다.
        """
        try:
            with self.motion_lock:
                point_count = int(request.point_count)
                flat_ticks = np.asarray(request.joint_ticks, dtype=np.int64)
                ...
                path = [
                    row.copy()
                    for row in flat_ticks.reshape(point_count, 5)
                ]
                ...
                velocity = (
                    int(request.profile_velocity)
                    if int(request.profile_velocity) > 0
                    else ARM_PROFILE_VELOCITY
                )
                timeout = (
                    float(request.timeout_sec)
                    if float(request.timeout_sec) > 0.0
                    else ARM_WAYPOINT_TIMEOUT_SEC
                )
    ...
    def handle_mike_gripper(
        ...
        """Move the gripper to an explicit tick and wait until it reaches it."""
        try:
            with self.motion_lock:
            ...
            response.message = str(error)
            try:
                response.reached_tick = int(
                    read_gripper_position(self.port, self.packet)
                )
            except Exception:
                response.reached_tick = 0
